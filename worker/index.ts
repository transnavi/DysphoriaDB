...
async function updateReaction(request: Request, env: Env, id: string): Promise<Response> {
  if (!validExperienceIds.has(id)) return json({ error: "Unknown experience" }, 404);
  ...
  return json({ id, selected: body.selected, count: Number(countResult.results[0]?.count ?? 0) });
}
  ...
  async fetch(request, env): Promise<Response> {
    ...
    try {
      ...
      if (match) {
        ...
        return updateReaction(request, env, id);
      }
