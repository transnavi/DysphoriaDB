...
import { buildSearchIndex, renderCatalog, renderExperienceDetail } from "../site/catalog-render.js";
import {
  createI18n,
  experiencePath,
  localeDefinitions,
...
function structuredData(i18n, locale, experience, canonicalUrl) {
  ...
  const localized = localizeExperience(i18n, experience, locale);
  graph.push(
    {
      "@type": "WebPage",
...
function localizeShell(baseHtml, i18n, locale, experience = null) {
  const definition = localeDefinitions[locale];
  const localized = experience ? localizeExperience(i18n, experience, locale) : null;
  const pathname = experience ? experiencePath(locale, experience.id) : localeRoot(locale);
  ...
  html = replaceMeta(html, "property", "og:type", experience ? "article" : "website");
  ...
  html = html.replace(
    ...
    `<script type="application/ld+json" id="structured-data">${jsonForHtml(structuredData(i18n, locale, experience, canonicalUrl))}</script>`,
  );
  ...
  if (!localized) {
    ...
    html = html.replace(
      ...
      `<div class="experience-detail" id="experience-detail">${renderExperienceDetail({ i18n, locale, experience, collection: experiences })}</div>`,
    );
  }
  return html;
}
...
for (const locale of locales) {
  ...
  for (const experience of experiences) {
    await writePage(experiencePath(locale, experience.id), localizeShell(baseHtml, i18n, locale, experience));
  }
}
...
const exportData = {
  ...
  experiences: experiences.map((experience) => ({
    ...
    content: Object.fromEntries(locales.map((locale) => {
      const localized = localizeExperience(i18nByLocale[locale], experience, locale);
      return [locale, {
...
const csvHeaders = ["id", "locale", "title", "summary", "domain", "family", "types", "directions", "tags", "report_count", "sources"];
